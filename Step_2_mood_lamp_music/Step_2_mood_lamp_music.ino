...
void loop() {
  ...
  for (float x = 0; x < 180; x = x + colourSpeed) {
    ...
    int difference = abs(lastMicLevel - mic_in);
     
    // save this value for the next round
    lastMicLevel = mic_in;

    // bring the "difference" into a useable range
    float mic_normalized = difference / 4.0;
    
    // make sure the mic_normalized value is between 0.2 and 1.0
    if (mic_normalized > 1.0) {
      mic_normalized = 1.0;
    }
    if (mic_normalized < 0.2) {
      mic_normalized = 0.2;
    }

    // Smooth out the volume bumps using a Low Pass Filter
    lastLowPass = mic_normalized * smoothWeight + (1 - smoothWeight) * lastLowPass;
